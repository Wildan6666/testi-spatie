...
import { Search, Download, Eye, ArrowLeft, Book, Building, FileText, ChevronRight } from "lucide-react";
import AOS from "aos";
...
export default function DetailDokumen() {
  ...
  //  Handler untuk filter sidebar
  const handleFilterChange = (name, value) => {
    setFilterState((prev) => ({ ...prev, [name]: value }));
  };

  const performSearch = () => {
    console.log("Mencari dokumen dengan kata kunci:", searchTerm);
  };

  const resetFilters = () => {
    setSearchTerm("");
    setFilterState({ jenis: "", tipe: "" });
  };
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center">
          <h2 className="text-xl font-bold text-red-500">Dokumen tidak ditemukan!</h2>
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="landing-container">
      ...
      {/* ====== Bagian Konten Utama ====== */}
      <div
        className="flex-1 bg-gradient-to-b from-[#fff9f5] via-[#f6bc96] via-[#f2a87e] to-[#d97b52]
      ...
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
          {/* ===== DETAIL UTAMA ===== */}
          <div className="lg:col-span-2 bg-white shadow-lg rounded-xl p-6" data-aos="fade-right">
            <div className="mb-6 border-b pb-4">
              <h2 className="text-2xl font-bold text-orange-700 mb-2">{doc.title}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              <div><span className="font-medium text-gray-700">Kategori:</span><p>{doc.kategori ?? "-"}</p></div>
              <div><span className="font-medium text-gray-700">Nomor Dokumen:</span><p>{doc.nomor ?? "-"}</p></div>
              <div><span className="font-medium text-gray-700">Status:</span>
                <p className={`font-semibold ${doc.status === "Berlaku" ? "text-green-600" : "text-red-600"}`}>
                  {doc.status ?? "-"}
                </p>
              </div>
              <div><span className="font-medium text-gray-700">Tanggal Penetapan:</span><p>{doc.date ?? "-"}</p></div>
              <div><span className="font-medium text-gray-700">Tanggal Pengundangan:</span><p>{doc.published ?? "-"}</p></div>
              <div><span className="font-medium text-gray-700">Jenis Hukum:</span><p>{doc.jenis ?? "-"}</p></div>
              <div><span className="font-medium text-gray-700">Tipe Dokumen:</span><p>{doc.type ?? "-"}</p></div>
              <div><span className="font-medium text-gray-700">Instansi:</span><p>{doc.instansi ?? "-"}</p></div>
            </div>

            {/* ===== STRUKTUR TREE ===== */}
            {(doc.ancestors?.length > 0 || doc.treeChildren?.length > 0) && (
              <div className="mt-8 bg-white border border-orange-200 rounded-xl p-6 shadow-sm" data-aos="fade-up">
                ...
                {doc.ancestors?.length > 0 && (
                  <div className="mb-6 bg-white border border-orange-200 rounded-xl p-5 shadow-sm" data-aos="fade-right">
                    <h3 className="text-sm font-semibold text-orange-700 mb-3 flex items-center gap-2">
                      <Book size={18} className="text-orange-600" /> Dokumen Induk
                    </h3>
                ...
                <div className="mb-6 bg-white border border-orange-200 rounded-xl p-5 shadow-sm" data-aos="fade-right">
                  <h3 className="text-sm font-semibold text-orange-700 mb-3 flex items-center gap-2">
                    <Book size={18} className="text-orange-600" /> Dokumen Turunan
                  </h3>
                  {doc.treeChildren?.length > 0 ? (
                    <TreeList nodes={doc.treeChildren} />
                  ) : (
                    <p className="text-gray-500 italic text-sm">Tidak ada dokumen turunan.</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="mt-6 flex items-center gap-6 text-sm text-gray-600">
              <div className="flex items-center gap-1"><Eye size={16} /> <span>{doc.views ?? 0}</span></div>
              <div className="flex items-center gap-1"><Download size={16} /> <span>{doc.downloads ?? 0}</span></div>
            </div>
